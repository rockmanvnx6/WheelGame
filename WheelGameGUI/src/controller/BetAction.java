...
public class BetAction implements ActionListener {
	...
	public boolean checkPlayersAddBet() {
		/* 
		 * This function will return true if all players has added bet,
		 * false otherwise.
		 * 
		 * Is used to auto-trigger the spin function when every one has placed
		 * their bets.
		 */
		...
		try {
			...
			if (popup.addBet(betTypes) == JOptionPane.OK_OPTION) {
				if (model.placeBet(currentPlayer, popup.getBet(), popup.getSelectedBet()) == false) {
					// If cannot place bet for any reasons.
					throw new Exception();
				} else {
